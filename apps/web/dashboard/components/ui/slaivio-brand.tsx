...
export function SlaivioBrand({ compact = false, inverse = false }: { compact?: boolean; inverse?: boolean }) {
  return (
    <span className="inline-flex min-w-0 items-center gap-2.5" aria-label="Slaivio">
      <Image
      ...
      />
      <span className={`${compact ? "text-[20px]" : "text-[27px]"} truncate font-semibold ${inverse ? "text-white" : "text-[#202428]"}`}>
        Slaivio
      </span>
    </span>
  );
