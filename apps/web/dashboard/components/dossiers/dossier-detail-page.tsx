  ...
  async function saveNewClient(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await createClientInDossier(dossierId, {
        ...
        lifecycle_status: clean(form.get("lifecycle_status")),
      });
...
function NewClientForm({ submit, error }: { submit: (event: FormEvent<HTMLFormElement>) => void; error: string }) { return <form id="new-dossier-client" onSubmit={submit} className="grid gap-5"><div className="rounded-[8px] bg-[#f3f6f5] px-4 py-3 text-[12px] leading-5 text-[#617069]">Le client sera automatiquement rattaché à ce dossier et recevra son identifiant SLAIVIO.</div><Field label="Nom complet"><input required autoFocus name="name" autoComplete="name" className={fieldClass} /></Field><div className="grid gap-4 sm:grid-cols-2"><Field label="Numéro de téléphone"><input required name="phone" inputMode="tel" autoComplete="tel" placeholder="Ex. +243 970 000 000" className={fieldClass} /></Field><Field label="Email — facultatif"><input name="email" type="email" autoComplete="email" className={fieldClass} /></Field></div><div className="grid gap-4 sm:grid-cols-2"><Field label="Type de client"><select required name="customer_type" defaultValue="individual" className={fieldClass}><option value="individual">Particulier</option><option value="business">Entreprise</option><option value="agent">Intermédiaire</option><option value="partner">Partenaire</option></select></Field><Field label="Statut du client"><select required name="lifecycle_status" defaultValue="lead" className={fieldClass}><option value="lead">Nouveau contact</option><option value="active">En relation active</option><option value="pending">En attente</option><option value="inactive">Inactif</option><option value="blocked">Ne plus contacter</option></select></Field></div>{error && <FormError text={error} />}</form>; }
