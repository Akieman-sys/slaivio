...
import { ArrowLeft, ChevronRight, MessageCircle, Plus, RotateCw, Users } from "lucide-react";
...
import {
  archiveDossier, attachClientToDossier, createClientInDossier, getDossier, listDossierMembers,
  removeClientFromDossier, restoreDossier, searchDossierClients, updateDossier, updateDossierClientRelation,
  type DossierClientRelation, type DossierClientSearchResult, type DossierMember, type DossierRecord,
} from "@/services/dossiers";

type DetailTab = "overview" | "clients" | "activity";
type ClientMode = "existing" | "new" | "edit";
...
export function DossierDetailPage({ dossierId }: { dossierId: string }) {
  ...
  async function saveNewClient(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await createClientInDossier(dossierId, {
        ...
        email: clean(form.get("email")), situation: clean(form.get("situation")),
      });
  ...
  function openEditClient(client: DossierClientRelation) { setClientMode("edit"); setEditingClient(client); setFormError(""); setClientOpen(true); }
  ...
  return <div className="min-h-full bg-[#f7f8f8] text-[#25292e]">
    ...
    <OperationContent className="mx-auto w-full max-w-[1180px]">
      ...
      {tab === "overview" ? <Overview dossier={dossier} /> : tab === "clients" ? <Clients dossier={dossier} edit={openEditClient} remove={removeClient} add={openAddClient} /> : <Activity dossier={dossier} />}
    </OperationContent>
    ...
    <OperationDrawer open={clientOpen} close={() => !saving && setClientOpen(false)} title={clientMode === "edit" ? "Mettre à jour le suivi" : "Ajouter un client"} description={clientMode === "edit" ? editingClient?.display_name : "Recherchez une personne existante ou créez une nouvelle fiche."} width="max-w-[640px]" footer={clientMode === "new" ? <><OperationButton disabled={saving} onClick={() => setClientOpen(false)}>Annuler</OperationButton><OperationButton variant="primary" type="submit" form="new-dossier-client" disabled={saving}>{saving ? "Ajout…" : "Créer et ajouter"}</OperationButton></> : clientMode === "edit" ? <><OperationButton disabled={saving} onClick={() => setClientOpen(false)}>Annuler</OperationButton><OperationButton variant="primary" type="submit" form="edit-dossier-client" disabled={saving}>{saving ? "Enregistrement…" : "Enregistrer"}</OperationButton></> : undefined}>
      {clientMode === "existing" ? <ExistingClientSearch query={query} setQuery={setQuery} searching={searching} matches={matches} select={attachExisting} create={() => setClientMode("new")} error={formError} /> : clientMode === "new" ? <NewClientForm submit={saveNewClient} error={formError} /> : editingClient ? <EditClientForm client={editingClient} submit={saveClientRelation} error={formError} /> : null}
    </OperationDrawer>
  </div>;
}
...
function Clients({ dossier, edit, remove, add }: { dossier: DossierRecord; edit: (client: DossierClientRelation) => void; remove: (client: DossierClientRelation) => void; add: () => void }) {
  ...
  return <div className="grid gap-3">{clients.map((client) => <article key={client.relation_id} className="rounded-[10px] border border-[#e0e4e7] bg-white p-5"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h2 className="text-[15px] font-semibold">{client.display_name}</h2>{client.relationship_role === "PRIMARY" && <OperationStatus label="Contact principal" tone="info" />}{client.attention_required && <OperationStatus label="À traiter" tone="warning" />}</div><p className="mt-1 text-[12px] text-[#77818b]">{client.client_reference}</p></div>{!dossier.archived_at && <PermissionGuard permission="dossiers.clients.manage"><div className="flex gap-2"><OperationButton onClick={() => edit(client)}>Mettre à jour</OperationButton><OperationButton variant="danger" onClick={() => remove(client)}>Retirer</OperationButton></div></PermissionGuard>}</div><dl className="mt-5 grid gap-4 border-t border-[#edf0f2] pt-4 sm:grid-cols-3"><Info label="Contact" value={client.phone || client.whatsapp_phone || client.email || "Non renseigné"} /><Info label="Situation" value={client.situation || "Non renseignée"} /><Info label="Dernière mise à jour" value={formatDateTime(client.last_updated_at)} /></dl>{client.attention_reason && <p className="mt-4 rounded-[7px] bg-[#fff7e8] px-3 py-2.5 text-[13px] text-[#805a1d]">{client.attention_reason}</p>}</article>)}</div>;
}
...
function NewClientForm({ submit, error }: { submit: (event: FormEvent<HTMLFormElement>) => void; error: string }) { return <form id="new-dossier-client" onSubmit={submit} className="grid gap-5"><div className="grid gap-4 sm:grid-cols-2"><Field label="Nom de la personne"><input name="name" className={fieldClass} /></Field><Field label="Entreprise"><input name="company_name" className={fieldClass} /></Field></div><div className="grid gap-4 sm:grid-cols-2"><Field label="Téléphone"><input name="phone" inputMode="tel" className={fieldClass} /></Field><Field label="Numéro WhatsApp"><input name="whatsapp_phone" inputMode="tel" className={fieldClass} /></Field></div><Field label="Email"><input name="email" type="email" className={fieldClass} /></Field><Field label="Situation dans ce dossier"><textarea name="situation" rows={4} className={`${fieldClass} h-auto py-2.5`} /></Field><p className="text-[12px] leading-5 text-[#74808a]">Renseignez un nom ou une entreprise, ainsi qu’au moins un moyen de contact.</p>{error && <FormError text={error} />}</form>; }
