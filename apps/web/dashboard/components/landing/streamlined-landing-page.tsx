    ...
    <main className="min-h-screen bg-[#fbfcfa] font-sans text-[#111814] antialiased">
      ...
      <SectionShell id="operations" eyebrow={t.operationsEyebrow} title={t.operationsTitle} text={t.operationsText}>
        <div className="mt-12 overflow-hidden rounded-xl border border-[#dce3dd] bg-white p-5 sm:p-8">
          ...
          <div className="mt-8 grid gap-5 border-t border-[#e7ebe8] pt-7 sm:grid-cols-3"><PreviewMetric label="Expéditions en cours" value="18" detail="12 dans les délais" /><PreviewMetric label="Colis en entrepôt" value="2 104" detail="184 reçus aujourd’hui" /><PreviewMetric label="À recevoir" value="6 680 $" detail="12 factures à échéance" /></div>
        </div>
      </SectionShell>
...
function ProductPreview() {
  const rows = [["EXP-2026-00458", "Guangzhou → Kinshasa", "En transit"], ["EXP-2026-00462", "Yiwu → Douala", "Préparation"], ["EXP-2026-00467", "Dubai → Kinshasa", "À valider"]];
...
function DemoModal({ t, step, status, close, next, back, submit }: { t: typeof copy.fr | typeof copy.en; step: number; status: "idle" | "loading" | "success" | "error"; close: () => void; next: (values: Record<string, string>) => void; back: () => void; submit: (event: FormEvent<HTMLFormElement>) => void }) {
  ...
  return <div className="fixed inset-0 z-[80] grid place-items-center bg-[#101713]/55 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label={t.formTitle}><div className="w-full max-w-[580px] rounded-xl border border-[#dce3dd] bg-white p-6 shadow-[0_30px_90px_rgba(15,23,42,.28)] sm:p-8"><div className="flex items-start justify-between"><div><p className="text-[12px] font-semibold uppercase tracking-[.12em] text-[#0f9e5a]">{t.formStep} {step}/2</p><h2 className="mt-2 text-[26px] font-semibold tracking-[-.035em]">{t.formTitle}</h2></div><button onClick={close} className="grid h-9 w-9 place-items-center rounded-md hover:bg-[#f1f3f1]" aria-label="Fermer"><X size={18} /></button></div>{status === "success" ? <div className="py-12 text-center"><span className="mx-auto grid h-12 w-12 place-items-center rounded-full bg-[#e7f8ef] text-[#0b9555]"><Check size={20} /></span><p className="mt-5 text-[15px] font-medium">{t.success}</p><button onClick={close} className="mt-6 h-10 rounded-md bg-[#111814] px-5 text-[14px] font-semibold text-white">OK</button></div> : step === 1 ? <form onSubmit={firstStep} className="mt-7 grid gap-4 sm:grid-cols-2"><Field label="Prénom"><input required name="first_name" className={field} /></Field><Field label="Nom"><input required name="last_name" className={field} /></Field><Field label="Email professionnel"><input required type="email" name="email" className={field} /></Field><Field label="Téléphone"><input required name="phone" className={field} /></Field><Field label="Agence"><input required name="agency_name" className={field} /></Field><Field label="Pays"><input required name="country" className={field} /></Field><button className="mt-2 h-11 rounded-md bg-[#111814] text-[14px] font-semibold text-white sm:col-span-2">{t.next}</button></form> : <form onSubmit={submit} className="mt-7 grid gap-4 sm:grid-cols-2"><Field label="Nombre d’employés"><select required name="employees" className={field}><option value="">Sélectionner</option><option>1–5</option><option>6–20</option><option>21–50</option><option>50+</option></select></Field><Field label="Volume mensuel"><select required name="monthly_shipments" className={field}><option value="">Sélectionner</option><option>Moins de 500 colis</option><option>500–2 000 colis</option><option>2 000–10 000 colis</option><option>Plus de 10 000 colis</option></select></Field><Field label="Nombre de bureaux"><input name="offices" type="number" min="1" className={field} /></Field><Field label="Outils actuels"><select name="tools" className={field}><option>WhatsApp + Excel</option><option>Logiciel existant</option><option>Email + Excel</option><option>Papier / cahiers</option><option>Autre</option></select></Field><Field label="Besoin principal" className="sm:col-span-2"><textarea required name="need" rows={3} className={`${field} h-auto py-3`} /></Field>{status === "error" && <p className="text-[13px] text-red-700 sm:col-span-2">{t.error}</p>}<div className="mt-2 flex gap-2 sm:col-span-2"><button type="button" onClick={back} className="h-11 flex-1 rounded-md border border-[#d3dad4] text-[14px] font-semibold">{t.back}</button><button disabled={status === "loading"} className="h-11 flex-[1.5] rounded-md bg-[#111814] text-[14px] font-semibold text-white disabled:opacity-60">{status === "loading" ? t.sending : t.send}</button></div></form>}</div></div>;
}
