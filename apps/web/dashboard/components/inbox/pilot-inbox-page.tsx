...
import { ArrowLeft, ArrowRight, CircleAlert, ExternalLink, MessageCircle, RotateCw, Send, UserRound } from "lucide-react";
...
import {
  getInboxConversation, listInboxConversations, markInboxConversationRead,
  sendInboxReply, updateInboxContext, updateInboxState,
  type InboxConversation, type InboxDetail, type InboxView,
} from "@/services/inbox";
...
export function PilotInboxPage() {
  ...
  useEffect(() => {
    const timer = window.setInterval(() => {
      void loadList(true);
      if (selectedPhone) void getInboxConversation(selectedPhone).then(setDetail).catch(() => undefined);
    }, 15000);
    return () => window.clearInterval(timer);
  }, [loadList, selectedPhone]);
  ...
  async function reply(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await sendInboxReply(detail.phone, message);
  ...
  return <div className="min-h-full bg-[#f7f8f8] text-[#25292e]">
    <OperationPageHeader title="Boîte de réception" description="Gérez les conversations WhatsApp de l’entreprise avec le client et son dossier au même endroit." />
    <OperationTabs>
      ...
      <section className="grid min-h-[650px] overflow-hidden rounded-[10px] border border-[#dfe3e6] bg-white shadow-[0_1px_2px_rgba(15,23,42,.03)] lg:h-[calc(100vh-225px)] lg:min-h-[620px] lg:grid-cols-[320px_minmax(400px,1fr)_300px]">
        ...
        <section className={`${selectedPhone ? "flex" : "hidden lg:flex"} min-h-0 flex-col`}>
          {!selectedPhone ? <EmptyConversation /> : detailLoading && !detail ? <LoadingState label="Ouverture de la conversation…" /> : detail ? <>
            <ConversationHeader detail={detail} back={() => { setSelectedPhone(null); setDetail(null); }} info={() => setContextOpen(true)} close={() => changeState(detail.assignment?.status === "CLOSED" ? "OPEN" : "CLOSED", false)} attention={() => changeState("OPEN", true)} />
            ...
            <ReplyComposer detail={detail} sending={sending} error={actionError} submit={reply} />
...
function ReplyComposer({ detail, sending, error, submit }: { detail: InboxDetail; sending: boolean; error: string; submit: (event: FormEvent<HTMLFormElement>) => void }) {
  ...
  return <form onSubmit={submit} className="border-t border-[#e2e6e9] bg-white p-3"><div className="mx-auto flex max-w-[820px] items-end gap-2"><textarea required name="message" rows={2} maxLength={4000} placeholder="Écrire une réponse au nom de l’entreprise…" className={`${fieldClass} min-h-[44px] flex-1 resize-none py-2.5`} /><PermissionGuard permission="inbox.reply"><OperationButton type="submit" variant="primary" className="h-10 w-10 px-0" disabled={sending} aria-label="Envoyer"><Send size={16} /></OperationButton></PermissionGuard></div>{error && <p className="mx-auto mt-2 max-w-[820px] text-[11px] text-[#b42318]">{error}</p>}</form>;
}
