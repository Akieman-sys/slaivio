...
function ClientProfileForm({ client, submit, error }: { client: DossierClientRelation; submit: (event: FormEvent<HTMLFormElement>) => void; error: string }) { return <form id="edit-client-profile" onSubmit={submit} className="grid gap-5"><div className="rounded-[8px] bg-[#f3f6f5] px-4 py-3 text-[12px] leading-5 text-[#617069]">Ces coordonnées appartiennent à la fiche unique du client. Le téléphone est aussi utilisé pour WhatsApp.</div><Field label="Identifiant client"><input value={client.client_reference} readOnly className={`${fieldClass} bg-[#f5f7f6] text-[#68737d]`} /></Field><Field label="Nom complet"><input required name="name" defaultValue={client.name || client.display_name} className={fieldClass} /></Field><Field label="Numéro de téléphone et WhatsApp"><input required name="phone" inputMode="tel" defaultValue={client.phone || client.whatsapp_phone || ""} className={fieldClass} /></Field><Field label="Email — facultatif"><input name="email" type="email" defaultValue={client.email || ""} className={fieldClass} /></Field>{error && <FormError text={error} />}</form>; }
...
function ClientRecord({ client }: { client: DossierClientRelation }) { return <section><h3 className="text-[15px] font-semibold">Fiche client</h3><dl className="mt-4 grid gap-5 sm:grid-cols-2"><Info label="Identifiant client" value={client.client_reference} /><Info label="Nom complet" value={client.name || client.display_name} /><Info label="Téléphone et WhatsApp" value={client.phone || client.whatsapp_phone || "Non renseigné"} /><Info label="Email" value={client.email || "Non renseigné"} /></dl></section>; }
