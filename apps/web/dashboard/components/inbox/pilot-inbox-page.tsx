...
const filters: Array<{ id: InboxView; label: string }> = [
  { id: "all", label: "Toutes" }, { id: "unread", label: "Non lues" },
...
export function PilotInboxPage() {
  ...
  return <div className="flex min-h-0 flex-col bg-[#f7f8f8] text-[#25292e] lg:h-[calc(100vh-64px)]">
    <OperationPageHeader title="Boîte de réception" description="Conversations WhatsApp, clients et dossiers réunis dans un espace de travail simple." actions={aiSettings && <AIModeControl settings={aiSettings} change={changeAgencyMode} />} />
    <main className="min-h-0 flex-1 p-3 sm:p-4"><section className="grid h-[calc(100vh-170px)] min-h-[620px] overflow-hidden rounded-[10px] border border-[#dfe3e6] bg-white shadow-[0_1px_2px_rgba(15,23,42,.04)] lg:h-full lg:min-h-0 lg:grid-cols-[330px_minmax(420px,1fr)_300px] xl:grid-cols-[350px_minmax(480px,1fr)_320px]">
      ...
      <section className={`${selectedPhone ? "flex" : "hidden lg:flex"} min-h-0 flex-col bg-white`}>{!selectedPhone ? <EmptyConversation /> : detailLoading && !detail ? <LoadingState label="Ouverture de la conversation…" /> : detail ? <>
        ...
        <ReplyComposer detail={detail} sending={sending} aiLoading={aiLoading} aiDisabled={(detail.assignment?.ai_mode_override || aiSettings?.pilot_response_mode) === "PAUSED"} error={actionError} value={replyText} change={setReplyText} suggest={askSuggestion} submit={reply} />
...
function ConversationRow({ item, active, checked, toggle, open }: { item: InboxConversation; active: boolean; checked: boolean; toggle: () => void; open: () => void }) {
  ...
  return <div className={`grid grid-cols-[28px_38px_minmax(0,1fr)_auto] items-center gap-2 border-b border-[#edf0f2] px-3 py-3 ${active ? "bg-[#eaf6f0]" : "hover:bg-[#f8faf9]"}`}><label className="grid h-7 w-7 place-items-center"><input type="checkbox" checked={checked} onChange={toggle} aria-label={`Sélectionner ${name}`} className="h-4 w-4 accent-[#087a46]" /></label><button onClick={open} className="contents text-left"><span className="grid h-9 w-9 place-items-center rounded-full bg-[#e3eee8] text-[12px] font-bold text-[#087a46]">{initials(name)}</span><span className="min-w-0"><span className="flex items-center gap-2"><strong className="truncate text-[13px]">{name}</strong>{item.unread_count > 0 && <span className="grid min-w-5 place-items-center rounded-full bg-[#087a46] px-1.5 text-[10px] font-bold text-white">{item.unread_count}</span>}</span><span className="mt-1 block truncate text-[12px] text-[#65717a]">{item.last_direction === "outbound" ? "Vous : " : ""}{item.last_message || "Pièce jointe"}</span><span className={`mt-1 inline-flex items-center gap-1 text-[10px] font-medium ${item.effective_ai_mode === "CONTROLLED_AUTO" ? "text-[#087a46]" : "text-[#7b858d]"}`}>{item.effective_ai_mode === "CONTROLLED_AUTO" ? <Bot size={10} /> : <UserRound size={10} />}{mode}</span></span><span className="self-start whitespace-nowrap pt-0.5 text-[10px] text-[#8a939b]">{relativeTime(item.last_message_at)}</span></button></div>;
}
