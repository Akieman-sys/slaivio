        ...
        rows = conn.execute(
            text(f"""
                select
                    ...
                    d.assigned_by,
                    d.due_at,
                    coalesce(to_jsonb(d)->>'whatsapp_group_status', 'DISABLED') whatsapp_group_status,
                    to_jsonb(d)->>'whatsapp_group_jid' whatsapp_group_jid,
                    to_jsonb(d)->>'whatsapp_group_created_at' whatsapp_group_created_at,
                    to_jsonb(d)->>'whatsapp_group_last_error' whatsapp_group_last_error,
                    coalesce((select (to_jsonb(org)->>'whatsapp_group_on_dossier_create')::boolean from organizations org where org.id = d.org_id), false) whatsapp_group_enabled,
                    d.created_at,
                    d.updated_at,
                    d.row_version,
                    d.archived_at,
